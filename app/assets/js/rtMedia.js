...
function apply_rtMagnificPopup( selector ) {
	jQuery( 'document' ).ready( function( $ ) {
		...
		if ( typeof( rtMediaData ) !== 'undefined' && typeof( rtMediaData.lightboxEnabled ) !== 'undefined' && rtMediaData.lightboxEnabled === '1' ) { // If lightbox is enabled.
...
jQuery( 'document' ).ready( function( $ ) {
	...
	jQuery( document ).ajaxComplete( function( event, xhr, settings ) {
		if ( 'legacy' !== rtMediaData.bpTemplatePack && rtMediaData.bpTemplatePack ) {
			var get_action = get_parameter( 'action', settings.data );
			if (('activity_filter' === get_action || 'post_update' === get_action || 'get_single_activity_content' === get_action || 'activity_get_older_updates' === get_action) && 'undefined' !== typeof rtMediaData && 'undefined' !== typeof rtMediaData.masonryLayout && 'true' === rtMediaData.masonryLayout && 'undefined' !== typeof rtMediaData.masonryLayoutActivity && 'true' === rtMediaData.masonryLayoutActivity ) {
				setTimeout( function() {
					apply_rtMagnificPopup( '.rtmedia-list-media.rtm-gallery-list, .rtmedia-activity-container ul.rtmedia-list, #bp-media-list,.bp-media-sc-list, li.media.album_updated ul,ul.bp-media-list-media, li.activity-item div.activity-content div.activity-inner div.bp_media_content, .rtm-bbp-container, ul.rtm-comment-container' );
	...
	$( '#rt_media_comment_form' ).submit( function( e ) {
		if ( $.trim( $( '#comment_content' ).val() ) == '' ) {
			if ( jQuery( '#rtmedia-single-media-container' ).length == 0 ) {
				rtmedia_gallery_action_alert_message( rtMediaData.emptyCommentMsg, 'warning' );
			} else {
				rtmedia_single_media_alert_message( rtMediaData.emptyCommentMsg, 'warning' );
			}
			return false;
		} else {
			return true;
	...
	// rtMediaData.lightboxEnabled from setting.
	...
	jQuery( '.rtmedia-container' ).on( 'click', '.select-all', function( e ) {
		...
		jQuery( this ).attr( 'title', rtMediaData.unselectAllVisible );
	...
	jQuery( '.rtmedia-container' ).on( 'click', '.unselect-all', function( e ) {
		...
		jQuery( this ).attr( 'title', rtMediaData.selectAllVisible );
		jQuery( '.rtmedia-list input' ).each( function() {
	...
	jQuery( '#rtmedia-create-album-modal' ).on( 'click', '#rtmedia_create_new_album', function( e ) {
		...
		if ( $albumname != '' ) {
			...
			$( '#rtmedia_create_new_album' ).prepend( '<img src=\'' + rtMediaData.loadingFile + '\' />' );
			jQuery.post( rtMediaData.ajaxurl, data, function( response ) {
				if ( typeof response.album != 'undefined' ) {
					...
					jQuery( '.rtmedia-user-album-list' ).each( function() {
						...
						if ( flag ) {
							var label = $context.charAt( 0 ).toUpperCase() + $context.slice( 1 ) + ' ' + rtMediaData.albumTitleText;
					...
					jQuery( '#rtmedia-create-album-modal' ).append( '<div class=\'rtmedia-success rtmedia-create-album-alert\'><b>' + $albumname + '</b>' + rtMediaData.albumCreatedMsg + '</div>' );
					...
					rtmedia_gallery_action_alert_message( rtMediaData.somethingWrongMsg, 'warning' );
				}
			...
			rtmedia_gallery_action_alert_message( rtMediaData.emptyAlbumNameMsg, 'warning' );
		}
	} );

	jQuery( '.rtmedia-container' ).on( 'click', '.rtmedia-delete-selected', function( e ) {
		if ( jQuery( '.rtmedia-list :checkbox:checked' ).length > 0 ) {
			if ( confirm( rtMediaData.selectedMediaDeleteConfirmation ) ) {
				jQuery( this ).closest( 'form' ).attr( 'action', '../../../' + rtMediaData.mediaSlug + '/delete' ).submit();
			}
		} else {
			rtmedia_gallery_action_alert_message( rtMediaData.noMediaSelected, 'warning' );
		}
	} );

	jQuery( '.rtmedia-container' ).on( 'click', '.rtmedia-move-selected', function( e ) {
		if ( jQuery( '.rtmedia-list :checkbox:checked' ).length > 0 ) {
			if ( confirm( rtMediaData.selectedMediaMoveConfirmation ) ) {
	...
	jQuery( '#buddypress' ).on( 'change', '.rtm-activity-privacy-opt', function() {
		...
		jQuery.post( ajaxurl, data, function( res ) {
			...
			var css_class = '';
			if ( res == 'true' ) {
				message = rtMediaData.privacyUpdateSuccess;
				css_class = 'success';
			} else {
				message = rtMediaData.privacyUpdateError;
				css_class = 'fail';
			}
	...
	jQuery( '#rtmedia-upload-container' ).after( '<div id=\'rtm-drop-files-title\'>' + rtMediaData.dropMediaMsg + '</div>' );
	if ( typeof rtMediaData !== 'undefined' && typeof rtMediaData.bpEnableActivity !== 'undefined' && rtMediaData.bpEnableActivity === '1' ) {
		jQuery( '#whats-new-textarea' ).append( '<div id=\'rtm-drop-files-title\'>' + rtMediaData.dropMediaMsg + '</div>' );
	}
	jQuery( document )
	...
	jQuery( '.rtmedia-container' ).on( 'click', '.rtmedia-delete-album', function( e ) {
		e.preventDefault();
		if ( confirm( rtMediaData.albumDeleteConfirmation ) ) {
			jQuery( this ).closest( 'form' ).submit();
		}
	} );
	...
	if ( typeof rtMediaData !== 'undefined' && typeof rtMediaData.masonryLayout !== 'undefined' && rtMediaData.masonryLayout == 'true' && typeof rtMediaData.masonryLayoutActivity != 'undefined' && rtMediaData.masonryLayoutActivity === 'true' ) {
	...
	jQuery( document ).ajaxComplete( function( event, xhr, settings ) {
		...
		if ( ( 'post_update' === get_action || 'get_single_activity_content' === get_action || 'activity_get_older_updates' === get_action ) && typeof rtMediaData !== 'undefined' && typeof rtMediaData.masonryLayout !== 'undefined' && rtMediaData.masonryLayout === 'true' && typeof rtMediaData.masonryLayoutActivity !== 'undefined' && rtMediaData.masonryLayoutActivity === 'true' ) {
			rtmedia_activity_masonry();
		}
	} );

	// Masonry code
	if ( typeof rtMediaData !== 'undefined' && typeof rtMediaData.masonryLayout !== 'undefined' && rtMediaData.masonryLayout === 'true' && jQuery( '.rtmedia-container .rtmedia-list.rtm-no-masonry' ).length === 0 ) {
	...
	jQuery( '.rtmedia-container' ).on( 'click', '.rtm-delete-media', function( e ) {
		e.preventDefault();
		var confirmation = rtMediaData.mediaDeleteConfirmation;
			...
			jQuery.ajax( {
				...
				success: function( response ) {
					...
					if ( 'rtmedia-media-deleted' === response.data.code ) {
						//Media delete
						rtmedia_gallery_action_alert_message( rtMediaData.mediaDeleteSuccess, 'success' );
						curr_li.remove();

						if ( typeof rtMediaData !== 'undefined' && 'undefined' !== typeof rtMediaData.masonryLayout && 'true' === rtMediaData.masonryLayout ) {
							rtm_masonry_reload( rtm_masonry_container );
						}
...
(function( $ ) {
 $.fn.shorten = function( settings ) {
     ...
     var config = {
         ...
         minHideChars: 10,
         ellipsesText: '...',
         moreText: rtMediaData.readMore,
         lessText: rtMediaData.showLess,
         onLess: function() {},
         onMore: function() {},
         errMsg: null,
         force: false
	...
	if ( jQuery( '#rtmedia_upload_terms_conditions' ).length > 0) {
		...
		jQuery( '#bp-nouveau-activity-form' ).on( 'click', '#aw-whats-new-submit', function ( event ) {
			...
			if ( 0 !== condition_checkbox.length && false === condition_checkbox.prop( 'checked' ) && form.find( '#message' ).length === 0 ) {
				...
				rtp_display_terms_warning( selector, rtMediaUploadTermsMainScriptData.checkTermsMessage );
			}
		});
