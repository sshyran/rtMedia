		...
		if ( have_rtmedia() ) :
			...
			<div id="rtmedia-single-media-container" class="rtmedia-single-media rtm-single-media rtm-media-type-<?php echo esc_attr( $media_type ); ?>">
				<?php if ( ! $rt_ajax_request ) { ?>
					...
					<div class="rtmedia-media" id="rtmedia-media-<?php echo esc_attr( rtmedia_id() ); ?>"><?php rtmedia_media( true ); ?></div>
				...
				<?php } else { ?>
					...
					<div class='rtm-ltb-action-container clearfix'>
						<div class='rtm-ltb-title'>
							<span
								class="rtmedia-media-name
								<?php
								if ( rtmedia_album_name() ) {
									?>
									rtmedia-media-name-width-50
									<?php
								} else {
									?>
									rtmedia-media-name-width-100<?php } ?>">
								&nbsp;<a href="<?php rtmedia_permalink(); ?>" title="<?php echo esc_attr( rtmedia_title() ); ?>"><?php echo esc_html( rtmedia_title() ); ?></a>
							</span>

							<?php if ( rtmedia_album_name() ) { ?>
								<span class="rtmedia-album-name">
									<span>&nbsp;<?php echo esc_html( apply_filters( 'rtmedia_update_under_album_text', __( 'under', 'buddypress-media' ) ) ); ?></span>
									<a href="<?php rtmedia_album_permalink(); ?>" title="<?php echo esc_attr( rtmedia_album_name() ); ?>"><?php echo esc_html( rtmedia_album_name() ); ?></a>
								</span>
							<?php } ?>
						</div>
